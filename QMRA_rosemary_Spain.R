...
reductions <- Normal$new("washing")$
    map_input("mu", Constant$new("mu_washing", -2.52))$
    map_input("sigma", Constant$new("sigma_washing", 0.12))

logN_stor <- ModulePlus$new("logN_stor")$
    map_input("a", logN0)$
...
quantile_table(exposure, chosen = "exposure", probs = c(.5, .1, .90))

quantile_table(Pill_adults, chosen = "Pill_adults", probs = c(.5, .1, .90)) %>%
    select(-node) %>%
    pivot_longer(everything()) %>%
    mutate(out = value*prevalence)

quantile_table(cases_adults, chosen = "cases", probs = c(.5, .1, .90)) %>%
    select(-node) %>%
    pivot_longer(everything()) %>%
...
quantile_table(Pill_kids, chosen = "Pill_kids", probs = c(.5, .1, .9)) %>%
    select(-node) %>%
    pivot_longer(everything()) %>%
    mutate(out = value*prevalence)

quantile_table(cases_kids, chosen = "cases", probs = c(.5, .1, .9)) %>%
    select(-node) %>%
    pivot_longer(everything()) %>%
    mutate(out = value/1e6*prevalence)
...
# ## Supp. Figure 1
# 
# rosemary <- exposure$simulations
# 
# p <- list(
#     `Rosemary EO solution` = rosemary,
#     `Oregano EO solution` = oregano,
#     `No washing` = nowashing,
#     `Commercial solution` = chlorine,
#     `Water washing` = water
#     ) %>%
#     map(., ~ sample_n(., 10000)) %>%
#     imap_dfr(., ~ mutate(.x, condition = .y)) %>%
#     mutate(condition = factor(condition, levels = c("No washing", "Water washing", 
#                                                     "Commercial solution", "Rosemary EO solution", "Oregano EO solution"))) %>%
#     ggplot() +
#     # geom_density(aes(logN, fill = condition), alpha = .5)
#     geom_boxplot(aes(x = condition, y = logN)) +
#     xlab("") + ylab("Microbial concentration at consumption (log CFU/g)") +
#     theme_gray(base_size = 14)
# 
# ggsave(p, filename = "supp_Figure_1.png",
#        width = 9, height = 6)
